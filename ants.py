...
class Place(object):
        ...
        if insect.is_ant:
            if self.ant is None:
                ...
                if self.ant.can_contain(insect):
                    self.ant = self.ant.contain_ant(insect)
                elif insect.can_contain(self.ant):
                    self.ant = insect.contain_ant(self.ant)
                else:
    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            # BEGIN Problem 13
            if isinstance(insect, QueenAnt) and not insect.constructed_instance:
                return 
            # END Problem 13
...
class Bee(Insect):
    ...
    damage = 1
    is_watersafe = True
    direction = True # True indicates forward movement while False indicates backwards movement
    already_Scared = False # True indicates already been scared and False represents that the bee hasn't been scared
    ...
    def action(self, colony):
        ...
        # BEGIN EC
        if self.direction == False and self.place.entrance:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        count = 0
        while the_place.entrance is not hive.entrance:
            if count < self.min_range:
                the_place = the_place.entrance
                count+=1
            if count == self.max_range:
                return random_or_none(the_place.bees)
            if the_place.bees:
                return random_or_none(the_place.bees)
            else:
                the_place = the_place.entrance
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        if self.armor - amount <= 0:
            ...
            for bee in bees_lst:
                bee = bee.reduce_armor(self.damage)
...
# END Problem 13
    ...
    def __init__(self, armor=1):
        # BEGIN Problem 13
        Ant.__init__(self, armor)
        self.constructed_instance = QueenAnt.constructed_value
        QueenAnt.constructed_value += 1
        # END Problem 13
    ...
    def action(self, colony):
        ...
        """
        # BEGIN Problem 13
        if self.constructed_instance == 0:
            ScubaThrower.action(self,colony)
            current_place = self.place.exit
            while current_place:
                if current_place.ant:
                    current_ant = current_place.ant
                    if not current_ant.buffed:
                        current_ant.damage = current_ant.damage * 2
                        current_ant.buffed = True 
                    if current_ant.is_container and current_ant.contained_ant and not current_ant.contained_ant.buffed:
                        current_ant.contained_ant.damage = current_ant.contained_ant.damage*2
                        current_ant.contained_ant.buffed = True
                current_place = current_place.exit


                
        else:
            self.armor = self.reduce_armor(self.armor)
    ...
    def reduce_armor(self, amount):
        ...
        if self.armor <= 0 and not self.constructed_instance:
            bees_win()
        self.place = self.place.remove_insect(self)
        return self.armor 
        # END Problem 13
